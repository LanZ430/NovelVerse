...
- **文生图能力**：基于场景描述生成图像
- **跨章节记忆**：保持角色和事件的连贯性，支持长篇小说的交互体验
- **进度控制**：可跳转章节、调整进度
- **模块化架构**：系统采用模块化设计，代码可维护性高，易于扩展
- **Web界面**：友好的用户交互界面
- **命令行支持**：支持命令行方式运行
...
- **InteractionAgent**：交互代理，处理用户与故事的交互
  - **SceneGenerator**：场景生成器，负责生成初始、普通和偏离结局场景
  - **InputEvaluator**：输入评估器，评估用户输入对故事发展的影响
  - **MemoryManager**：记忆管理器，管理和检索长期记忆和重要事件
  - **ImageGenerator**：图像生成器，根据场景描述生成图像
  - **StateManager**：状态管理器，管理交互状态的保存和加载
- **ImageService**：图像服务，提供文生图功能

## 模块化设计

本系统采用模块化设计，特别是交互代理（InteractionAgent）部分被拆分为多个功能模块，以提高代码可维护性和扩展性：

- **场景生成模块**：负责根据上下文和用户输入生成不同类型的场景，包括初始场景、交互场景和偏离结局场景
- **输入评估模块**：评估用户输入的情绪、意图和对情节的潜在影响，计算故事偏离度
- **记忆管理模块**：管理长期记忆和跨章节记忆，为交互提供连贯的上下文支持
- **图像生成模块**：根据场景描述生成符合上下文的图像，支持多种艺术风格
- **状态管理模块**：负责交互状态的保存和加载，支持交互进度控制和恢复

该模块化架构确保了各部分职责清晰，便于团队协作开发和未来功能扩展。同时通过适配器模式保持与现有代码的兼容性，确保系统正常运行。
